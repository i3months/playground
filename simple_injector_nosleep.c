/* 
 * simple_injector_nosleep.c - No SINGLESTEP, just run and inject
 */
...
int main(int argc, char *argv[]) {
    ...
    if (target_pid == 0) {
        ...
        // Just continue - let it run
        ptrace(PTRACE_CONT, target_pid, 0, 0);
        
        // Wait a bit (child is running)
        usleep(1000 + (rand() % 10000));  // 1-11ms
        
        // Kill and restart with fault
        kill(target_pid, SIGKILL);
        waitpid(target_pid, &status, 0);
        
        // Start again with fault injection at beginning
        target_pid = fork();
